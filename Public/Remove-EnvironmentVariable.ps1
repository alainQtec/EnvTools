function Remove-EnvironmentVariable {
    ...
    .EXAMPLE
        ...
        Remove-EnvironmentVariable -VariableName 'bob'

    .EXAMPLE
        >
        # Remove an environment variable from Machine
        Remove-EnvironmentVariable -VariableName 'bob' -VariableType 'Machine'
    ...
    if ($variableType -eq [System.EnvironmentVariableTarget]::Machine) {
        if ($IsAdmin) {
            Add-EnvironmentVariable -Name $variableName -Value $null -Scope $variableType
        } else {
            Write-Warning "$fxn : [!]  It seems You're not Admin [!] "
            $psArgs = "Add-EnvironmentVariable -Name `'$variableName`' -Value $null -variableType `'$variableType`'"
