<#
.SYNOPSIS
    EnvTools buildScript
.DESCRIPTION
    A custom Psake buildScript for the module EnvTools.
.LINK
    https://github.com/alainQtec/EnvTools/blob/main/build.ps1
.EXAMPLE
...
Begin {
    ...
    $dataFile = [System.IO.FileInfo]::new([IO.Path]::Combine($PSScriptRoot, 'en-US', 'EnvTools.strings.psd1'))
    if (!$dataFile.Exists) { throw [System.IO.FileNotFoundException]::new('Unable to find the LocalizedData file.', 'EnvTools.strings.psd1') }
